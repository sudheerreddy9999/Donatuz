...
const BookingCards = ({ currentOrders, handleJoinCall, createRoomId }: Props) => {
  ...
  const CallDetails = ({ call }: { call: BookingData }) => (
    <div className="space-y-1 hidden md:block">
      ...
      <div className="flex justify-between text-[13px]">
        ...
        <p className="text-gray-300">${call.amountPaid}</p>
      </div>
      <div className="flex justify-between text-[13px]">
        <span className="font-medium">Payment method</span>
        <p className="text-gray-300">{call.paymentMethod}</p>
      </div>
  ...
  const CallStatus = ({ call }: { call: BookingData }) => (
    <p
  ...
  const CallTimeLeft = ({ call }: { call: BookingData }) => (
    <p
      className={`text-[15px] md:text-[14px] ml-8 md:ml-0 ${
        call.status === "Active"
          ? "text-green-400"
          : call.status === "upcomming"
          ? "text-blue-400"
  ...
  const CallActions = ({ call }: { call: BookingData }) => (
    <div className="flex justify-between mt-2 text-xs">
      <div>
        <button
          className={`border border-white md:border-none ${
            call.status != "History" && "md:bg-red-500 px-10 py-2 md:px-6"
          } rounded-md`}
        >
      ...
      {call.status !== "History" ? (
        <div className="flex">
          ...
          <Link href={`/videoCall/${createRoomId}`}>
            <button
              ...
              onClick={call.status === "Active" ? handleJoin : undefined}
        ...
        <div className="flex space-x-3 justify-center items-center text-sm">
          ...
          <StarRating
            ...
            onRatingChange={(newRating) => handleRatingChange(call.id, newRating)}
          />
        </div>
      )}
    </div>
    ...
    <div>
      {currentOrders.map((call) => (
        <div key={`item-${call.id}`} className="rounded-md text-sm m-2">
          ...
          <div
            className={`bg-[#ffffff1a] backdrop-blur-[11.6px] m-1 md:m-0 p-2 md:p-3 rounded-md space-y-2 ${
              !isMdScreen && call.status == "Active" && "hidden"
            }`}
          ...
          >
            <div className="flex md:flex-col justify-start items-start md:justify-between md:items-stretch">
              <div className="flex justify-between">
                <div className="flex text-xs md:text-[14px] space-x-1 md:space-x-3 justify-center items-center">
                  <img
                    src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQn9zilY2Yu2hc19pDZFxgWDTUDy5DId7ITqA&s"
                    className="h-[60px] w-[60px] md:h-8 md:w-8 rounded-md md:rounded-full"
                    alt="User"
                  />
                  <div className="space-y-2 text-gray-300 p-1 md:p-0">
                    <span className="md:font-medium text-[14px] md:text-[13px] md:text-gray-300">
                      {call.name}
                    </span>
              ...
              <div className="flex justify-between">
                {call.status!=='History' && <CallTimeLeft call={call} />}
              </div>
            </div>
            ...
            <div className="w-full">
              ...
              {expandedItems[call.id] && <CallDetails call={call} />}
